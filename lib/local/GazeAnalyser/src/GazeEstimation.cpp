...
cv::Point2f GazeAnalysis::GetScreenCM(cv::KalmanFilter &kalmanX, cv::Mat &stateX, cv::KalmanFilter &kalmanY, cv::Mat &stateY, const LandmarkDetector::CLNF& clnf_model, cv::Point3f &gaze_vector_1, cv::Point3f &gaze_vector_2, const float fx, const float fy, const float cx, const float cy) {
	float eyes_z = 0;
	int part = -1;
	for (size_t i = 0; i < clnf_model.hierarchical_models.size(); ++i)
	{
		...
		cv::Mat measurementX = cv::Mat::zeros(1, 1, CV_32F);
		cv::Mat measurementY = cv::Mat::zeros(1, 1, CV_32F);
		...
		measurementX += kalmanX.measurementMatrix * stateX;
		measurementY += kalmanY.measurementMatrix * stateY;
